...
-- 테이블 : invoices
SELECT CustomerId, COUNT(*) AS invoices_cnt
FROM invoices
GROUP BY CustomerId
HAVING COUNT(*) >= 7
;
...
SELECT ins.InvoiceId, ins.Total, cu.FirstName, cu.LastName
...
LEFT JOIN albums
    ON artists.ArtistId = albums.ArtistId
WHERE albums.AlbumId IS NULL -- 앨범이 하나라도 있는 아티스트를 뽑으려면 IS NOT NULL
;
