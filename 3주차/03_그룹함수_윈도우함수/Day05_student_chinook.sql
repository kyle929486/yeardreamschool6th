...
SELECT
    ...
    BillingCountry,
    Total,
    AVG(Total) OVER (PARTITION BY BillingCountry) AS CountryAvgTotal
FROM invoices
...
-- LAG, LEAD 함수 활용
---- LAG(Total, 1) : 뒤의 숫자만큼 NULL 값 생성됨 (위에서부터)
---- LEAD(Total, 1) : 뒤의 숫자만큼 NULL 값 생성됨 (아래에서부터)
SELECT
    InvoiceId,
    InvoiceDate,
    Total,
...
SELECT
    ...
    ROUND(AVG(t.UnitPrice), 2)  AS AvgPrice    -- 장르와 미디어타입별로 평균 단가(AvgPrice)
FROM tracks t
INNER JOIN genres g
